...
import { validateCategoryFields } from "./Valadation"
import { LogsFunction } from "../functions"
import { EDIT_BLOG_CATEGORY } from "../others/Messages"
...
function EditBlogCategory() {
    ...
    async function handleSubmit() {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_URL}/blog-category/${id}`, {
                method: "PATCH",
                headers: {
            ...
            if (res.status === 200) {
                ...
                LogsFunction(EDIT_BLOG_CATEGORY + state.name)
                if (response.success) {
                ...
                {loading ? <Loading /> :
                    <div className="create-blog-body">
                        <div className="d-flex row">
                            ...
                            {state.bottomBlock && state.bottomBlock.length > 0 && (
                            ...
                            {state.internalBlock && state.internalBlock.length > 0 && (
                                <div className=" input-text faq-preview mx-2">
                        ...
                        <div className="d-flex justify-content-end mt-4">
                            <button className="purplebtn" type="button" onClick={handleSubmit}>Edit Blog Category</button>
                        </div>
                    </div>
                }
